package org.foomaa.jvchat.globaldefines;


@SuppressWarnings("unused")
public class JvColorsAnsiGlobalDefines {
    JvColorsAnsiGlobalDefines() {}

    public final String RESET = "\033[0m";
    public final String BLACK = "\033[0;30m";
    public final String RED = "\033[0;31m";
    public final String GREEN = "\033[0;32m";
    public final String YELLOW = "\033[0;33m";
    public final String BLUE = "\033[0;34m";
    public final String PURPLE = "\033[0;35m";
    public final String CYAN = "\033[0;36m";
    public final String WHITE = "\033[0;37m";
    public final String BLACK_BOLD = "\033[1;30m";
    public final String RED_BOLD = "\033[1;31m";
    public final String GREEN_BOLD = "\033[1;32m";
    public final String YELLOW_BOLD = "\033[1;33m";
    public final String BLUE_BOLD = "\033[1;34m";
    public final String PURPLE_BOLD = "\033[1;35m";
    public final String CYAN_BOLD = "\033[1;36m";
    public final String WHITE_BOLD = "\033[1;37m";
    public final String BLACK_UNDERLINED = "\033[4;30m";
    public final String RED_UNDERLINED = "\033[4;31m";
    public final String GREEN_UNDERLINED = "\033[4;32m";
    public final String YELLOW_UNDERLINED = "\033[4;33m";
    public final String BLUE_UNDERLINED = "\033[4;34m";
    public final String PURPLE_UNDERLINED = "\033[4;35m";
    public final String CYAN_UNDERLINED = "\033[4;36m";
    public final String WHITE_UNDERLINED = "\033[4;37m";
    public final String BLACK_BACKGROUND = "\033[40m";
    public final String RED_BACKGROUND = "\033[41m";
    public final String GREEN_BACKGROUND = "\033[42m";
    public final String YELLOW_BACKGROUND = "\033[43m";
    public final String BLUE_BACKGROUND = "\033[44m";
    public final String PURPLE_BACKGROUND = "\033[45m";
    public final String CYAN_BACKGROUND = "\033[46m";
    public final String WHITE_BACKGROUND = "\033[47m";
    public final String BLACK_BRIGHT = "\033[0;90m";
    public final String RED_BRIGHT = "\033[0;91m";
    public final String GREEN_BRIGHT = "\033[0;92m";
    public final String YELLOW_BRIGHT = "\033[0;93m";
    public final String BLUE_BRIGHT = "\033[0;94m";
    public final String PURPLE_BRIGHT = "\033[0;95m";
    public final String CYAN_BRIGHT = "\033[0;96m";
    public final String WHITE_BRIGHT = "\033[0;97m";
    public final String BLACK_BOLD_BRIGHT = "\033[1;90m";
    public final String RED_BOLD_BRIGHT = "\033[1;91m";
    public final String GREEN_BOLD_BRIGHT = "\033[1;92m";
    public final String YELLOW_BOLD_BRIGHT = "\033[1;93m";
    public final String ANSI_GREEN = "\u001B[32m";
    public final String ANSI_YELLOW = "\u001B[33m";
    public final String ANSI_BLUE = "\u001B[34m";
    public final String ANSI_PURPLE = "\u001B[35m";
    public final String ANSI_CYAN = "\u001B[36m";
    public final String BLUE_BOLD_BRIGHT = "\033[1;94m";
    public final String PURPLE_BOLD_BRIGHT = "\033[1;95m";
    public final String CYAN_BOLD_BRIGHT = "\033[1;96m";
    public final String WHITE_BOLD_BRIGHT = "\033[1;97m";
    public final String BLACK_BACKGROUND_BRIGHT = "\033[0;100m";
    public final String RED_BACKGROUND_BRIGHT = "\033[0;101m";
    public final String GREEN_BACKGROUND_BRIGHT = "\033[0;102m";
    public final String YELLOW_BACKGROUND_BRIGHT = "\033[0;103m";
    public final String BLUE_BACKGROUND_BRIGHT = "\033[0;104m";
    public final String PURPLE_BACKGROUND_BRIGHT = "\033[0;105m";
    public final String CYAN_BACKGROUND_BRIGHT = "\033[0;106m";
    public final String WHITE_BACKGROUND_BRIGHT = "\033[0;107m";
    public final String ANSI_RESET = "\u001B[0m";
    public final String ANSI_BLACK = "\u001B[30m";
    public final String ANSI_RED = "\u001B[31m";
    public final String ANSI_WHITE = "\u001B[37m";
}